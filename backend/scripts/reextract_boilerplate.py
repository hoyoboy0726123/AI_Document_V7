"""重抽「內容含 everyspec 浮水印／頁首樣板」的文件（_strip_boilerplate 修正後）。
...
from app.models import DocumentChunk as _DC  # noqa: E402,F811

_CID_RE = re.compile(r"\(cid:\d+\)")


def boilerplate_chunks(db, doc_id: str) -> int:
    """該文件有多少區塊仍含浮水印／單獨成行的文件編號。"""
    return db.query(_DC).filter(
        _DC.document_id == doc_id,
        _DC.text.like("%everyspec%"),
    ).count()
...
def main() -> None:
    ...
    for d, n in sorted(targets, key=lambda x: -x[1]):
        ...
        print(f"  {(d.title or '')[:44]:<46} 含浮水印 {n:>4} 塊　現有區塊 {cnt}")
    ...
    backup_dir = args.backup_dir or f"backup_boiler_{datetime.now():%Y%m%d_%H%M%S}"
