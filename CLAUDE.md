...
AI Document V7 is a document management + OCR + RAG Q&A system with multimodal answers. Two independent processes:
...
- **LLM provider abstraction** (`services/llm_provider/`): a provider-neutral `LLMProvider` Protocol (`base.py`) with `OllamaProvider` and `AiHubProvider` impls (Gemini was removed in V6), selected by the factory in `__init__.py`. Text LLM can be `ollama` or `aihub` (ASUS AiHub OpenAPI gateway, GPT-OSS 120B; **no streaming at the gateway level** — the UI pushes retrieval sources early instead). Embedding/vision are always ollama. `AIHUB_API_KEY` is read from `.env` only — never from the admin UI or DB. Resolution order: `system_configs` DB row → `.env` → fallback `ollama`. Providers are cached singletons keyed by a config fingerprint; **call `llm_provider.invalidate()` after admin saves provider settings** (the admin endpoint and `main.py::apply_llm_overrides_from_db()` mutate the in-memory `settings` object). New Agent/KG code uses this abstraction; legacy call sites still use `ollama_client.get_client()` directly — migrate incrementally, don't rip them out.
...
- **KG auto-extraction** (`services/kg_auto.py`, admin page 表格關係抽取): two LLM-assisted, verification-gated builders. (1) Table auto-suggest: LLM proposes {worth?, parent_name, id/label columns} per detected table, the deterministic `kg_tables` engine writes rows; code-set-overlap grouping merges OCR-fragmented tables under one parent with `meta.aliases` (routing and `list_subitems` match aliases too). Suggested model: a large local one (qwen3.8:27b) — the 8B default misjudges. (2) Prose citation extraction: per-chunk LLM extraction of external spec IDs, each claim literally verified against the chunk text (paren/whitespace/s2tw-tolerant) before becoming a `references` edge (evidence prefix `llm_citation:` enables batch rollback via DELETE /kg/citations/{doc}). Manual, background-task, never at ingest.
- **Citation-to-page highlighting** (`services/pdf_locate.py`, POST /documents/{id}/page/{n}/locate): returns PDF-point rects for a snippet — text PDFs via word-trigram matching over `page.get_text("words")` (NOT `search_for`, which fails across line breaks), image PDFs via on-the-fly single-page RapidOCR line boxes. Frontend overlays highlight rects in `PdfPreviewModal` (prop `highlightSnippet`).
- **Rich answers** (frontend `components/QA/richContent.jsx`): mermaid rendering with label auto-quoting hardening + raw-code fallback; strict numeric-table→ECharts opt-in button; KG force-graph mini-views. Relation/procedure/enumeration early exits attach `kg_graph` to the final event (persisted via `conversations.append_message(kg_graph=...)`).
