...
AI Document V3 is a document management + OCR + RAG Q&A system. Two independent processes:
...
- **Schema migrations**: there is **no Alembic yet**. `main.py::ensure_schema_updates()` runs ad-hoc `ALTER TABLE` on every startup (idempotent, gated by `inspector.get_columns`). When adding columns to existing tables, also add a guarded `ALTER TABLE` branch here, otherwise existing dev DBs will diverge from `models.py`. New tables (e.g. `kg_entities`, `kg_relations`) are created via `Base.metadata.create_all()`.
