# AI_Document_V7 — 軍規標準智慧問答系統（RAG × 知識圖譜 × 多模態）

> **V7 開發中。** 主軸是多模態回答介面：引用原文頁高亮溯源（文字型與圖片型 PDF 通吃的座標覆蓋層）、答案內圖表/圖譜視覺化、HTML/投影片 artifacts。
> V7 由 V6 完整複製而來並保留全部 commit 歷史（起點 bb70c6d，V6 封存 tag：v6-final）。
...
## V6→V7 已落地的能力（此複本包含全部）

| 能力 | 說明 |
|---|---|
| 雲端文字模型 | ASUS AiHub OpenAPI（GPT-OSS 120B 自建雲端），與本地 Ollama 可於管理介面切換；嵌入/視覺仍走本地。API 金鑰僅從 .env 讀取 |
| KG 感知路由 | 使用者自建的圖譜母節點自動可路由、可列舉（不再依賴名詞白名單 regex） |
| 表格自動抽取 | LLM 當設定產生器、規則引擎逐列執行、完整性由建構保證；OCR 切散的同批表自動合併（別名機制）。建議模型 qwen3.8:27b |
| 散文引用抽取 | LLM 逐塊抽引用編號 + 原文字面查核，查核不過不入圖；管理者手動觸發、可整批回退 |
| 引用溯源高亮 | 點來源→原文頁高亮被引用段落；文字型 PDF 毫秒級、圖片型現場 OCR 座標（兩型通吃） |
| 答案富內容 | Mermaid 圖（自動補引號硬化）、數值表一鍵轉圖表（嚴格判定）、關聯/程序/列舉題自動附 KG 力導向小圖 |
...
之所以不是 MIT，是因為相依的 **PyMuPDF 是 AGPL-3.0**，與本專案構成單一作品。
AGPL 第 13 條要求：**只要有人透過網路使用本服務（含公司區網內的同事），
就必須讓他們能取得完整原始碼。** 本 repo 公開且包含完整可建置的原始碼，
前端側邊欄底部也固定顯示原始碼連結，即為此要求的落實。

著作權人保留隨時以其他條款另行授權的權利（AGPL 約束的是取得者，不是著作權人）。

相依套件的完整授權清單、PyMuPDF 的使用範圍，以及日後若要脫離 AGPL 的
替代方向，見 **[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)**。
