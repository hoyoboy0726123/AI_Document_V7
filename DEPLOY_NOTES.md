...
> **2026-08-12 更新**：已由 `services/citation_check.py` 解決。答案生成後
> 比對每個 [來源N] 段落與來源內容的詞重疊（確定性字串比對），錯標且有
> 明確更佳來源（支持度 ≥0.40 且高出原引用 ≥0.15）時改標。只動標記、
> 不碰內容。實測「buying mode有幾種」四處 [來源4]→[來源2]（支持度
> 0.05~0.19 → 1.00），golden set u14 引用正確率 0→1.00，其餘 13 題
> 逐題零回歸。開關：`RAG_CITATION_CHECK`（預設開）。
> 純 RAG 串流已逐字送出收不回來，修正版隨 sources 事件的
> `corrected_answer` 送出，前端整段替換（只有編號變）。
>
> 以下保留原始診斷紀錄：
